...
public class BobstoryDao {
	...
	public List<BobstoryVo> selectMyBobstory(Connection conn, String no, PageVo pv) {
		...
		String sql = "SELECT * FROM (SELECT ROWNUM AS RNUM , AB.* FROM (SELECT B.NO , B.CATEGORY , B.TITLE , B.CONTENT , B.ENROLL_DATE , B.DELETE_YN , B.C_LIKE , B.MODIFY_DATE , B.VIEW_COUNT , B.REPORT_YN , M.NICK AS WRITER FROM BOBSTORY B JOIN MEMBER M ON B.WRITER = M.NO WHERE WRITER = ? AND DELETE_YN = 'N'ORDER BY B.NO DESC) AB ) WHERE RNUM BETWEEN ? AND ?";
