              ...
              <hr><div style="font-size: 20px; font-weight: 700; ">[ 기타 ]</div>
		        ...
		        <div id="page-area">
		        	...
		        	<%if(pv.getStartPage() != 1){%>
			        	<a href="/cookTeacher/product/main/etcList?pno=<%= pv.getStartPage()-1 %>" class="btn btn-primary btn-sm">이전</a>
		        	<%}%>
		        	
		            <!-- 페이지 버튼 -->
		        	<%for(int i = pv.getStartPage(); i<= pv.getEndPage(); ++i){%>
			            <a href="/cookTeacher/product/main/etcList?pno=<%= i %>" class="btn btn-primary btn-sm"><%= i %></a>
		        	<%}%>
		        	
		        	<!-- 다음 버튼 -->
		        	<%if(pv.getEndPage() != pv.getMaxPage()){%>
					    <a href="/cookTeacher/product/main/etcList?pno=<%= pv.getEndPage()+1 %>" class="btn btn-primary btn-sm">다음</a>
		       		<%}%>
